								...
								for (int i = 1 ; i <= MaxClients ; i++)
								{
									if(IsValidClient(i,false) && i!=victim)
									{
										...
										if(GetClientTeam(i)==GetClientTeam(victim) && dist < 256)
										{
											...
											target_pos[2] += 41;
											bool hit = false;
											Handle hndl = TR_TraceRayFilterEx(damagePosition, target_pos, MASK_SOLID, RayType_EndPoint, TraceFilter, i);
											if(TR_DidHit() == false)
												hit = true;
											else
											{
...
public Action HeatmakerSub(Handle timer, int iClient)
{
	if(IsValidClient(iClient))
	{
		...
		if(primaryIndex==752) //heatmaker cost
		{
			...
			{
				float meter = GetEntPropFloat(iClient, Prop_Send, "m_flRageMeter");
				PrintToChat(iClient,"%.2f,%.2f,%.2f",meter,meter>20 ? meter-20 : 0.0,TF2Util_GetPlayerConditionDuration(iClient,TFCond_FocusBuff));
				...
				if(IsPlayerAlive(idx) && GetClientTeam(idx) != GetClientTeam(client))
				{
					...
					if(distance < 130.0)
					{
						...
						if(CalcViewsOffset(angles1, angles2) < limit)
						{
							// TR_TraceRayFilter(player_pos, target_pos, MASK_SOLID, RayType_EndPoint, TraceFilter, idx);
							bool hit = false;
							Handle hndl = TR_TraceRayFilterEx(player_pos, target_pos, MASK_SOLID, RayType_EndPoint, TraceFilter, idx);
							if(TR_DidHit() == false)
								hit = true;
							else
							{
