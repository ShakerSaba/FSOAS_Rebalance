...
public Action PlayerSpawn(Handle timer, DataPack dPack)
{
	...
	if (IsValidClient(iClient) && g_bEnablePlugin.BoolValue)
	{
		...
		switch(TF2_GetPlayerClass(iClient))
		{
			...
			// {
			// 	TF2Attrib_SetByDefIndex(iClient,93,0.75); //Construction rate decreased
		...
		switch(primaryIndex)
		{
			...
			case 1151:
			{
				TF2Attrib_SetByDefIndex(primary,1,0.85); //damage penalty
			...
			// {
			// 	TF2Attrib_SetByDefIndex(primary,97,0.85); //Reload time decreased
			// 	TF2Attrib_SetByDefIndex(primary,793,1.0); //hype on damage
		...
		switch(secondaryIndex)
		{
			...
			case 159, 433:
			{
				...
				TF2Attrib_SetByDefIndex(secondary,800,1.0); //patient overheal penalty
			}
			//Jarate, Mad Milk
			...
			case 595:
			{
				...
				TF2Attrib_SetByDefIndex(secondary,1,1.5); //damage penalty
				TF2Attrib_SetByDefIndex(secondary,74,0.6); //weapon burn time reduced
				TF2Attrib_SetByDefIndex(secondary,869,1.0); //crits become minicrits
			}
			//Quick-Fix
			case 411:
			{
		...
		switch(meleeIndex)
		{
			case 349,43,357,416,38,1000,457,813,834,307,132,172,266,327,404,482,1082,155,232,450,37,1003,413,447,173:
			{
				...
				switch(meleeIndex) //specific melee changes
				{
					...
					case 173:
					{
						TF2Attrib_SetByDefIndex(melee,811,0.01); //ubercharge_preserved
						// TF2Attrib_SetByDefIndex(melee,144,0.0); //set weapon mode
						...
						if(strcmp(event,"player_spawn") == 0)
						{
							g_meterMel[iClient] = 0.0+GetEntProp(iClient, Prop_Send, "m_iHealPoints");
						}
					}
				}
			}
			default:
			{
				...
				switch(meleeIndex) //other melee changes
				{
					...
					case 310:
					{
						...
						TF2Attrib_SetByDefIndex(melee,412,1.0); //dmg taken increased
						TF2Attrib_SetByDefIndex(melee,264,1.5); //melee range multiplier
						TF2Attrib_SetByDefIndex(melee,773,1.67); //deploy time increased
						// TF2Attrib_SetByDefIndex(melee,5,1.25); //fire rate penalty
					}
					//The Equalizer
					case 128:
...
public Action Event_PlayerDeath(Event event, const char[] cName, bool dontBroadcast)
{
	...
	// if(TFClass_Pyro == TF2_GetPlayerClass(victim))
	// {
	// 	int vicmelee = TF2Util_GetPlayerLoadoutEntity(victim, TFWeaponSlot_Melee, true);
	// 	int vicmeleeIndex = -1;
	// 	if(vicmelee != -1) vicmeleeIndex = GetEntProp(vicmelee, Prop_Send, "m_iItemDefinitionIndex");
	// 	if(vicmeleeIndex == 813 || vicmeleeIndex == 834) //neon kill afterburn effects
	// 	{
	// 		EmitSoundToClient(victim,"player/flame_out.wav");
	// 		for (int i = 1 ; i <= MaxClients ; i++)
	// 		{
	// 			if(IsValidClient(i))
	// 			{
	// 				if(IsPlayerAlive(i) && TF2Util_GetPlayerConditionProvider(i,TFCond_OnFire)==victim)
	// 				{
	// 					TF2_RemoveCondition(i,TFCond_OnFire);
	// 					EmitSoundToClient(i,"player/flame_out.wav");
	// 				}
	// 			}
	// 		}
	// 	}
...
public void OnGameFrame()
{
				...
				case TFClass_Scout:
				{
					...
					switch(primaryIndex)
					{
						...
						case 448:
						{
							...
							// ShowHudText(iClient,2,"%.0f JUMPS",g_meterPri[iClient]);
						}
					}
				}
				case TFClass_Pyro:
				...
				case TFClass_Medic:
				{
					...
					switch(meleeIndex)
					{
						...
						case 173:
						{
							...
							if(meter >= 100)
							{
								ShowHudText(iClient,4,"Organ: DONE",meter);
	...
	// 		PrintToChatAll("%d: %d %d %.2f",CART,level,cappers,recede);
	// 		SetVariantInt(2*cappers);
				...
				if(curr==primary)
				{
					...
					else
					{
						...
						if(reload!=0)
						{
							...
							{
								int newClip = ammoCount-maxClip < 0 ? (ammoCount-maxClip+clip > maxClip ? maxClip : ammoCount-0+clip) : maxClip;
		...
		if(damagetype & DMG_CLUB && GetClientTeam(victim) == GetClientTeam(attacker) && victim != attacker)
		{
			...
			if(meleeIndex == 173)
			{
				...
				if(organs>0)
				{
					...
					{
						case TFClass_Spy,TFClass_Scout,TFClass_Sniper,TFClass_Engineer: health += 125.0;
						case TFClass_Medic: health += 150.0;
						case TFClass_Pyro,TFClass_DemoMan: health += 175.0;
						case TFClass_Soldier: health += 200.0;
						case TFClass_Heavy: health += 300.0;
					}
					//get health mods
					switch(vicPriIndex)
					{
					...
					switch(vicSecIndex)
					{
						case 226: health+=20;
						case 231: health+=15;
						case 159,433: health+=TF2Attrib_GetValue(TF2Attrib_GetByDefIndex(vicSec,26));
					...
					switch(vicMelIndex)
					{
						case 44: health+=-15;
						case 142: health+=25;
						case 356: health+=TF2Attrib_GetValue(TF2Attrib_GetByDefIndex(vicMel,125));
					...
					while(organs>0 && amount<amountToHeal)
					{
						...
						// amount = amount+100 > amountToHeal ? amountToHeal : amount+100;
					}
	...
	if(IsValidClient(attacker))
	{
		...
		switch(tfAttackerClass)
		{
			...
			case TFClass_Pyro:
			{
				...
				// if(DMG_BURN & damagetype)
				// {
				// int curr = GetEntPropEnt(attacker, Prop_Send, "m_hActiveWeapon");
				// float heal = damage/2.0 > 1.0 ? damage/2.0 : 1.0;
				// SetHudTextParams(0.1, -0.16, 1.0, 255, 255, 255, 255);
				// ShowHudText(attacker,5,"+%.0f HP",heal);
				// TF2Util_TakeHealth(attacker,heal);
				// }
			}
		}
		switch(weaponIndex)
...
public Action OnTakeDamage(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
		...
		switch(tfAttackerClass)
		{
			...
			case TFClass_DemoMan:
			{
				if(weaponIndex == 307 && damagetype & DMG_BLAST == DMG_BLAST) //caber damage bonus just on explosion
					damage *= 1.11;
				if(weaponIndex == 1151 && (damage == 85.0 || damage == 255.0 || damage == 114.75)) //restore direct damage from iron bomber
					damage /= 0.85;
			}
			case TFClass_Scout:
			{
			...
			case TFClass_Medic:
			{
				switch(weaponIndex){
					...
					{
						// int healing = GetEntProp(attacker, Prop_Send, "m_iHealPoints");
						// float meter = (healing-g_meterMel[attacker])/6.0;
						// if(meter < 0) meter = 0.0;
						// if(meter > 100) meter = 100.0;

						// float mult = 1 + 2*meter/100.0;
						// damage *= mult;
						// g_meterMel[attacker] = 0.0+healing;
	...
	// PrintToChatAll("%.2f %d %d %d %d",damage,victim,attacker,weapon,inflictor);
	...
	// PrintToChatAll("%d->%d %.2f",attacker,victim,getPlayerDistance(attacker,victim));
	return Plugin_Changed;
}
...
Action BuildingThink(int building,int client)
{
	...
	{
		// if(StrEqual(class,"obj_teleporter") && IS_MEDIEVAL)
		// {
		// 	SetVariantInt(999);
		// 	AcceptEntityInput(building,"RemoveHealth");
		// }
		...
		{
			//speed up health gain while building
			bool tele = StrEqual(class,"obj_teleporter"); //tele needs faster resources
			g_buildingHeal[building] += 1;
			int interval = 24; //not boosted
			switch(rate)
			{
				case 2.5: interval = 9; //wrench boost
				case 2.94: interval = 6; //jag boost
				case 1.74: interval = 12; //EE boost
			}
			if(tele) interval /= 3;
			if(g_buildingHeal[building]%interval==0)
			{
				SetVariantInt(1);
...
Action BuildingDamage (int building, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3])
{
	if(weapon != -1)
	{
		...
		switch(weaponIndex)
		{
			...
			case 1151:
			{
				if (damage == 85.0) //restore direct damage from iron bomber
					damage /= 0.85;
			}
		}
...
public Action PackDelete(Handle timer, int pack)
{
	...
	if(StrEqual(class,"item_healthkit_small"))
	{
...
public Action chocolateHeal(Handle timer, int iClient)
{
	...
	if(IsValidClient(iClient))
	{
		if(IsPlayerAlive(iClient) && (secondaryIndex == 159 || secondaryIndex == 433))
		{
			...
			TF2Attrib_SetByDefIndex(secondary,800,0.57143); //patient overheal penalty
		}
	}
	return Plugin_Continue;
}
			...
			case 2:
			{
				...
				for(int i=0; i<MaxClients; ++i) //find teammates nearby
				{
					if(IsValidClient(i))
					{
						...
						if(flagTeam == clientTeam && IsPlayerAlive(i))
						{
							if(getPlayerDistance(flag,i)<216)
							{
...
Action FlameTouch(int flame, int other) //lingering flamethrower flames
{
	...
	if(other==0 && g_lastFire[owner]+0.045<time && primaryIndex!=215)
	{
		...
		case 17,204,36,412:
		{
			if (other >= 1 && other <= MaxClients)
			{
				...
				if(other != owner && TF2_GetClientTeam(owner) == team)
				{
					...
					{
						SetEntDataFloat(other,8968,dmgTime-3.0,true); //reduce by 3 seconds on-hit
