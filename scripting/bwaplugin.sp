...
int FOOL_TIME = 1743537600; //Tue Apr 01 2025 20:00:00 GMT+0000
...
#define TF_CONDFLAG_OBJECT			(1 << 28)
...
public void OnPluginStart()
{
	...
	StartPrepSDKCall(SDKCall_Entity);
	PrepSDKCall_SetFromConf(data, SDKConf_Virtual, "CTFGrenadePipebombProjectile::Detonate");
	g_hSDKDetonate = EndPrepSDKCall();
...
public void OnClientPutInServer (int iClient)
{
	...
	SDKHook(iClient, SDKHook_StartTouch, PlayerTouch);
}
...
public void OnMapStart()
{
	...
	PrecacheSound("weapons/pan/melee_frying_pan_02.wav",true);
	PrecacheSound("weapons/pan/melee_frying_pan_03.wav",true);
	PrecacheSound("weapons/pan/melee_frying_pan_04.wav",true);
	PrecacheSound("player/cyoa_pda_beep6.wav",true);
...
public void TF2Items_OnGiveNamedItem_Post(int iClient, char[] cName, int itemIndex, int itemLevel, int itemQuality, int item)
{
	...
	if((!IsFakeClient(iClient) || !g_bIsMVM) && notDisguise)
	{
		...
		if (StrContains(cName,"_bat")>=0||StrContains(cName,"saxxy")>=0||StrContains(cName,"shovel")>=0||StrContains(cName,"katana")>=0||
			StrContains(cName,"fireaxe")>=0||StrContains(cName,"slap")>=0||StrContains(cName,"breakable")>=0||StrContains(cName,"bottle")>=0||
		...
		switch(itemIndex)
		{
			...
			case 414:
			{
				TF2Attrib_SetByDefIndex(item,6,0.85); //fire rate bonus
			...
			case 232:
			{
				...
				TF2Attrib_SetByDefIndex(item,412,1.0); //dmg taken increased
				TF2Attrib_SetByDefIndex(item,773,1.35); //deploy time increased
			...
			case 297: //Enthusiast's Timepiece 
			{
				if(GetTime()>FOOL_TIME)
				{
					...
					TF2Attrib_SetByDefIndex(item,729,0.0); //ReducedCloakFromAmmo
					TF2Attrib_SetByDefIndex(item,82,1.4); //cloak consume rate increased
				}
			}
...
public Action Event_PlayerDeath(Event event, const char[] cName, bool dontBroadcast)
{
	...
	if(IsValidClient(attacker))
	{
		if(customKill!=TF_CUSTOM_DEFENSIVE_STICKY && customKill!=TF_CUSTOM_AIR_STICKY_BURST && customKill!=TF_CUSTOM_STANDARD_STICKY)
		{
			...
			if(TF2_GetPlayerClass(attacker)==TFClass_Scout && meleeIndex==349)
			{
				//Sun-on-a-stick fill HEAT on kill
			...
			if (IsValidClient(i))
			{
				...
				if(GetTime()>FOOL_TIME && meleeIndex==1127) //Crossing Guard
				{
					...
					if(IsValidEdict(g_TRAIN[iClient]))
					{
						...
						trainExists = false;
					if(lastFire-GetGameTime()<delay+0.5 && lastFire-GetGameTime()>delay && !(g_condFlags[iClient]&TF_CONDFLAG_INFIRE) && !trainExists)
					{
				...
				{
					float lastFire = GetEntPropFloat(melee, Prop_Send, "m_flNextPrimaryAttack");
					float delay = TFClass_Scout==TF2_GetPlayerClass(iClient) ? 0.3 : 0.66;
					float vel[3];
					GetEntPropVector(iClient, Prop_Data, "m_vecAbsVelocity", vel);
					if(lastFire-GetGameTime()<delay+0.5 && lastFire-GetGameTime()>delay && !(g_condFlags[iClient]&TF_CONDFLAG_INFIRE))
					{
						if(!((clientFlags & FL_ONGROUND) && vel[2]==0.0))
						{
							float pos[3],ang[3];
						...
						switch(meleeIndex)
						{
							...
							case 349: //Sun-on-a-stick
							{
								...
								if(g_condFlags[iClient] & TF_CONDFLAG_HEAT)
								{
									...
									CreateParticle(iClient,"flaming_arrow",0.001,_,_,eyePos[0]-curPos[0],eyePos[1]-curPos[1],eyePos[2]-curPos[2],_,_,_,true);
								}
					...
					case TFClass_Soldier:
					{
						switch(primaryIndex)
						{
							case 441: //cow mangler adjust speed
							{
								...
								else //check charge level not being reset
								{
									if(g_meterPri[iClient]>0)
										g_meterPri[iClient] = 0.0;
								}
							}
						}
					...
					case TFClass_Pyro:
					{
						if(primaryIndex != 594 && primaryIndex !=1 && primary!=-1) //handle airblast, except for phlog
						{
							...
							if(weaponState!=2 && weaponState!=3 && nextAttack-0.15<GetGameTime()) //out of airblast
							{
								...
								else if(primaryIndex==215) increment *= 1.5; //degreaser faster recharge
						...
						switch(primaryIndex)
						{
							...
							case 298: //Iron Curtain
							{
								...
								if(weaponState==3)
								{
									...
									{
										SetHudTextParams(-1.0, -0.4, 0.1, 255, 255, 255, 255);
										ShowHudText(iClient,2,"%.0f%% CHARGE",(diff > 4.0 ? 4.0 : diff)*100/4.0);
									}
								}
							}
							default:
					...
					case TFClass_Sniper:
					{
						...
						if(primaryIndex!=56 && primaryIndex!=1005 && primaryIndex!=1092)
						{
							...
							if(reload!=0)
							{
								...
								else if(primaryIndex==851 && GetTime()>FOOL_TIME) reloadSpeed=3.7;
								float clientPos[3];
								...
								if(GetTime()>FOOL_TIME)
								{
									if(TF2_IsPlayerInCondition(iClient,TFCond_Cloaked))
									{
										...
										if(!TF2_IsPlayerInCondition(iClient,TFCond_Jarated))
											TF2_AddCondition(iClient,TFCond_Jarated);
	...
	{	
		...
		switch(tfClientClass)
		{
				...
				if(1179 == secondaryIndex)
				{
					...
					if((currVel[2]!=0.0 || !(clientFlags & FL_ONGROUND)) && !rocketing && regen > 50)
					{
						if ((buttons & IN_JUMP) && !(g_LastButtons[iClient] & IN_JUMP) && g_meterSec[iClient] >= 0.125)
						{
							...
							currVel[0] = ((vel[0] * Cosine(angle)) - (-vel[1] * Sine(angle)))*0.50; //scale horizontal force
							currVel[1] = ((-vel[1] * Cosine(angle)) + (vel[0] * Sine(angle)))*0.50; //scale horizontal force
							currVel[2] = 400.0; //fixed height
			...
			case TFClass_Heavy:
			{
				...
				if(primary != -1)
				{
					...
					if(primaryIndex==298)
					{
						...
						if(weaponState==2 && time-g_meterPri[iClient]>0.88)
						{
							float diff = time-g_meterPri[iClient]-0.88;
							float power = 8.0 + (diff > 4.0 ? 4.0 : diff)*2.0;
							if(diff<1.0) power *= (2-diff);
							TF2Attrib_SetByDefIndex(primary,2,power); //damage bonus
			...
			case TFClass_Scout:
			{
				...
				else if(meleeIndex==452)//Three-Rune Blade
				{
					if(GetTime()>FOOL_TIME)
					{
						if(TF2_IsPlayerInCondition(iClient,TFCond_Charging))
		...
		{
			//reset loch reload speed
			Address addr = TF2Attrib_GetByDefIndex(iClient,177);
			if(addr != Address_Null)
			{
				float val = TF2Attrib_GetValue(addr);
				if(val==0.5715 && weapon!=0)
...
public Action OnTraceAttack(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &ammotype, int hitbox, int hitgroup)
{
	...
	if(IsValidClient(attacker) && !tank)
	{
		...
		if (damagetype & DMG_CLUB && GetClientTeam(victim) != GetClientTeam(attacker))
		{
			...
			{
				if(g_consecHits[attacker]%3==1 && meleeIndex != 1181) //minicrit, exclude from Hot Hand
				{
					TF2_AddCondition(victim,TFCond_MarkedForDeathSilent,0.015);
				}
			}
	...
	switch(tfVictimClass)
	{
		...
		case TFClass_Sniper,TFClass_Spy:
		{
			...
			if(secondaryIndex == 231 || (meleeIndex==649 && g_condFlags[victim] & TF_CONDFLAG_INFIRE))
			{
				...
				if(damage==4.0 || damage==3.0 || damage==5.4 || damage==4.05) isBleed=false;
				if(isBleed)
				{
					//get bleed duration
					float duration=0.0;
	...
	if(IsValidClient(attacker))
	{
		...
		switch(weaponIndex)
		{
			case 41: //natascha heal on hit
			{
				...
				{
					float healing = dist < 256.0 ? 9.0 : 8.0*((1024-dist)/768.0) + 1.0;
			...
			case 441: //mangler burn for lower charge
			{
				...
				{
					if(g_manglerCharge[inflictor] > 0 && g_manglerCharge[inflictor] < 3) //is projectile the charged one
					{
						if(TF2_GetPlayerClass(victim) != TFClass_Pyro) TF2Util_SetPlayerBurnDuration(victim,6.0);
					}
				}
			}
			case 173: //vita-saw effect
			{
	...
	if(IsValidEdict(weapon) && IsValidClient(attacker))
	{
		...
		if(StrEqual("tf_weapon_flamethrower",weaponName) && (damagetype & DMG_IGNITE) && !(damagetype & DMG_BLAST) && damagecustom != TF_CUSTOM_TAUNT_ARMAGEDDON && !tank)
		{
			...
			if(damagetype & DMG_CRIT)
			{
				...
				{
					if(weaponIndex==594 || weaponIndex==40 || weaponIndex==1146)  //scale Phlog && Backburner damage
					{
		...
		switch(tfAttackerClass)
		{
			...
			case TFClass_DemoMan:
			{
				...
				if(weaponIndex == 308) //loch-n-load dets other stickies
				{
					//on direct hit
					if(damage == 100.0 || damage == 135.0 || damage == 300.0)
					{
						DataPack pack = new DataPack();
						pack.Reset();
						pack.WriteCell(attacker);
						pack.WriteFloatArray(damagePosition,3);
						RequestFrame(AutoDetStickies,pack);
					}
				}
				...
				switch(weaponIndex)
				{
					...
					case 413: //solemn vow knockback
					{
						if(!tank)
						{
							...
							{
								vicForce[0] = unitForce[0]*300; vicForce[1] = unitForce[1]*300; vicForce[2]= 350.0;
							...
							{
								atkForce[0] = unitForce[0]*-300; atkForce[1] = unitForce[1]*-300; atkForce[2]= 350.0;
		...
		if(!tank)
		{
			...
			{
				if((damagetype & DMG_BLAST && (weaponIndex == 127)) || (damagetype & DMG_BUCKSHOT && (weaponIndex == 415)))
				{
					if(!(damagetype & DMG_CRIT))
					{
						damagetype |= DMG_CRIT;
		...
		switch(victimClass)
		{
			...
			case TFClass_Sniper:
			{
				...
				if(weaponIndex != 460 && weaponIndex != 171 && secondaryIndex == 57)
				{
					...
					if(g_condFlags[victim]&TF_CONDFLAG_RAZOR==TF_CONDFLAG_RAZOR)
					{
			...
			case TFClass_Soldier,TFClass_DemoMan:
			{
				if(victim!=attacker && victim!=inflictor)
				{
					...
					int secondaryIndex = -1;
					if(secondary>0) secondaryIndex = GetEntProp(secondary, Prop_Send, "m_iItemDefinitionIndex");
					if(TF2_IsPlayerInCondition(victim,TFCond_Parachute) && (secondaryIndex == 1101 || primaryIndex == 1101 || (GetTime()>FOOL_TIME && TF2_GetPlayerClass(victim)==TFClass_Sniper)))
					{
...
public Action FireDebuff(Handle timer, DataPack pack)
{
	...
	if(g_condFlags[victim] & TF_CONDFLAG_ALWAYS && GetTime()>FOOL_TIME)
	{
		SetCommandFlags("r_screenoverlay", GetCommandFlags("r_screenoverlay") & ~FCVAR_CHEAT);
		ClientCommand(victim, "r_screenoverlay \"models/props_island/rainbow\"");
		ClientCommand(victim, "fade");
		// ClientCommand(victim, "r_screenoverlay \"models/props_island/rainbow\"");
		SetCommandFlags("r_screenoverlay", GetCommandFlags("r_screenoverlay") | FCVAR_CHEAT);
	}
	return Plugin_Continue;
								...
								if(CalcViewsOffset(angles1, angles2) < limit)
								{
									...
									if(TR_DidHit() == false)
									{
										if(idx <= MaxClients)
										{
											...
											{
												SDKHooks_TakeDamage(idx, entity, client, 20.0, DMG_SHOCK, entity, NULL_VECTOR, target_pos, false);
												extraCharge++;
											}
											else{
...
public void AutoDetStickies(DataPack pack)
{
	pack.Reset();
	int attacker = pack.ReadCell();
	float pos[3],pipePos[3];
	pack.ReadFloatArray(pos,3);
	//go through loch user's stickies
	int ent = -1;
	while ((ent = FindEntityByClassname(ent, "tf_projectile_pipe_remote")) != -1)
	{
		GetEntPropVector(ent, Prop_Send, "m_vecOrigin", pipePos);
		int weapon = GetEntPropEnt(ent, Prop_Send, "m_hLauncher");
		int owner = GetEntPropEnt(weapon, Prop_Send, "m_hOwnerEntity");
		if(GetVectorDistance(pos,pipePos)<120 && owner==attacker)
		{
			float dmg = GetEntPropFloat(ent, Prop_Send, "m_flDamage");
			SetEntPropFloat(ent, Prop_Send, "m_flDamage", dmg*1.2);
			SDKCall(g_hSDKDetonate, ent);
		}
	}
}
			...
			if(IsValidClient(owner))
			{
				if (other != owner && other >= 1 && other <= MaxClients)
				{
					...
					if(TF2_GetClientTeam(owner) == team)
					{
						...
						{
							SetEntDataFloat(other,8968,currTime-10.0,true); //set last damage time back
...
void AirblastPush(int client,float base)
{
	...
	if (primaryIndex==40||primaryIndex==1146) //backburner force
		...
		meter *= 0.00;
	else if(primaryIndex==741 && GetTime()>FOOL_TIME)//The Rainblower
		meter *= 1.50;
	if(g_bIsMVM) //mvm force
	{
		Address addr = TF2Attrib_GetByName(primary, "melee range multiplier");
...
void manglerAdjust(int entity)
{
	if(IsValidEdict(entity))
	{
		...
		if (StrEqual(class, "tf_projectile_energy_ball"))
		{
			...
			if(charged==1)
			{
				if(g_meterPri[owner]>=1)
				{
					vel[0]*=1.50;vel[1]*=1.50;vel[2]*=1.50;
				}
				if(g_meterPri[owner]!=3)
				{
					SetEntProp(entity, Prop_Send, "m_bChargedShot", 0);
				}
				g_manglerCharge[entity] = RoundFloat(g_meterPri[owner]);
			}
...
void orbSpawn(int entity)
{
	...
	if (StrEqual(class, "tf_projectile_mechanicalarmorb"))
	{
		...
		vel[0]*= 0.25 + 0.75*(g_meterSec[owner]/1.9);
		vel[1]*= 0.25 + 0.75*(g_meterSec[owner]/1.9);
		vel[2]*= 0.25 + 0.75*(g_meterSec[owner]/1.9);
		TeleportEntity(entity,_,_,vel);
		// destroy short circuit orb much sooner
		CreateTimer(0.5+0.5*(g_meterSec[owner]/1.9),KillOrb,entity);
		SetEntPropFloat(weapon, Prop_Send, "m_flNextSecondaryAttack",GetGameTime()+0.6);
		g_meterSec[owner] = 2.0;
	}
}
	...
	if(IsValidClient(iClient) && GetTime()>FOOL_TIME)
	{
		...
		if(meleeIndex==1127)
		{
			...
			if(train != -1)
			{
				...
				SDKHook(train, SDKHook_SpawnPost, TrainPostSpawn);
				...
				pos[0]+=offset[0]; pos[1]+=offset[1]; pos[2]+=offset[2];
				vel[0] = (vel[0]/len)*2000;
				vel[1] = (vel[1]/len)*2000;
...
Action PlayerTouch(int entity, int other)
{
	if(IsValidClient(entity) && IsValidClient(other))
	{
		...
		int watchIndex = -1;
		if(watch >= 0) watchIndex = GetEntProp(watch, Prop_Send, "m_iItemDefinitionIndex");
		if(watchIndex==297 && GetTime()>FOOL_TIME && TF2_IsPlayerInCondition(entity,TFCond_Cloaked))
		{
			if(GetClientTeam(entity)!=GetClientTeam(other) && IsPlayerAlive(entity) && IsPlayerAlive(other) && !TF2_IsPlayerInCondition(other,TFCond_Ubercharged))
			{
				float pos[3];
				GetEntPropVector(entity, Prop_Send, "m_vecOrigin", pos);
				EmitAmbientSound("vo/spy_sf12_goodmagic06.mp3",pos,entity);
