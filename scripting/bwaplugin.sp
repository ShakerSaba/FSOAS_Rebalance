...
public Action PlayerSpawn(Handle timer, DataPack dPack)
{
	...
	if (IsValidClient(iClient) && g_bEnablePlugin.BoolValue)
	{
		...
		{
			TF2Attrib_SetByDefIndex(secondary,106,0.9); //weapon spread bonus
		}
		...
		switch(primaryIndex)
		{
			...
			case 811,832:
			{
				...
				TF2Attrib_SetByDefIndex(primary,431,2.0); //spinup ammo drain
			}
			//Baby Face's Blaster
			case 772:
			{
		...
		switch(tfClientClass)
		{
			...
			case TFClass_Heavy:
			{
				...
				if(311 == secondaryIndex || 159 == secondaryIndex || 433 == secondaryIndex)//steak and dalokohs stuff
				{
					...
					if(buttons & IN_ATTACK && curr == secondary && nextAtk<=(GetGameTime()+0.045)) //setup steak and chocolate heals
					{
						g_condFlags[iClient] |= TF_CONDFLAG_EATING;
