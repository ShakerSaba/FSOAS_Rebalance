...
public void TF2Items_OnGiveNamedItem_Post(int iClient, char[] cName, int itemIndex, int itemLevel, int itemQuality, int item)
{
	...
	if((!IsFakeClient(iClient) || !g_bIsMVM) && notDisguise)
	{
		...
		switch(itemIndex)
		{
			...
			case 308:
			{
				TF2Attrib_SetByDefIndex(item,96,1.25); //Reload time increased
...
public Action Event_FlagEvent(Event event, const char[] cName, bool dontBroadcast)
{
	...
	if(IsValidClient(player) && (type==TF_FLAGEVENT_PICKEDUP || type==TF_FLAGEVENT_DEFENDED))
	{
...
public void OnGameFrame()
{
	...
	for (int i = 1; i <= MaxClients; i++)
	{
		...
		if (IsValidClient(i,false) && IsPlayerAlive(i))
		{
			...
			// PrintToChat(iClient,"%.2f",GetEntDataFloat(iClient,LAST_DAMAGE));
								...
								else
								{
									if(g_meterPri[iClient]>=1.0)
									{
										...
										if(clip<max && ammoCount>0)
										{
											...
											EmitSoundToClient(iClient,"weapons/rocket_reload.wav");
										}
									}
									else
		...
		switch(weaponIndex)
		{
			...
			case 304: //amputator heal radius on-hit
			{
				if(damage>0 && (valid | tank))
				{
					...
					{
						PrintToChat(attacker,"START RADIUS");
						DataPack pack = new DataPack();
						pack.Reset();
						pack.WriteCell(attacker);
						pack.WriteCell(19);
				...
				if(IsValidClient(i,false))
				{
					if(IsPlayerAlive(i) && (GetClientTeam(i)==GetClientTeam(healer) || (TF2_GetPlayerClass(i)==TFClass_Spy && TF2_IsPlayerInCondition(i,TFCond_Disguised))))
					{
						...
						if(getPlayerDistance(i,healer) <= 450)
						{
							...
							float dmgTime = GetEntDataFloat(i,LAST_DAMAGE); //m_flLastDamageTime
							...
							if(CurrHP<MaxHP)
							{
								...
								PrintToChat(healer,"%d amount healed %.2f",cycles-19,amount);
